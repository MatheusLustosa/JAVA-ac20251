...
public class ApoliceMediator {
    ...
    public RetornoInclusaoApolice incluirApolice(DadosVeiculo dados) {
        if (dados == null) return new RetornoInclusaoApolice(null, "Dados do ve�culo devem ser informados");
        if (StringUtils.ehNuloOuBranco(dados.getPlaca())) return new RetornoInclusaoApolice(null, "Placa do ve�culo deve ser informada");
        ...
        if (isCpf && !ValidadorCpfCnpj.ehCpfValido(cpfOuCnpj)) return new RetornoInclusaoApolice(null, "CPF inv�lido");
        if (isCnpj && !ValidadorCpfCnpj.ehCnpjValido(cpfOuCnpj)) return new RetornoInclusaoApolice(null, "CNPJ inv�lido");
        ...
        if (categoria == null) return new RetornoInclusaoApolice(null, "Categoria inv�lida");
        ...
        if (valorInformado.compareTo(limiteInferior) < 0 || valorInformado.compareTo(limiteSuperior) > 0) {
            return new RetornoInclusaoApolice(null, "Valor m�ximo segurado deve estar entre 75% e 100% do valor do carro encontrado na categoria");
        }
    ...
    public String excluirApolice(String numero) {
        if (StringUtils.ehNuloOuBranco(numero)) return "N�mero deve ser informado";

        Apolice apolice = daoApo.buscar(numero);
        if (apolice == null) return "Ap�lice inexistente";
